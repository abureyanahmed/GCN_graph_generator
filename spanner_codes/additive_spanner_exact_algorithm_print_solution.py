...
def write_file(file_name, levels, nodes, edges, Ts):
...
def single_flow_ilp(G, subset_arr, checker, param, folder_name, file_name_without_ext):
  my_inf = 1000000
  #print('Graph:', end=' ')
  #print_edges(G)
  #print(subset_arr)
  ...
  edges = []
  for j in range(numcols):
    if names[j].count('_')==2:
      #print("Column %s:  Value = %10f" % (names[j], x[j]))
      if x[j] >= .5:
        name_split = names[j].split('_')
        u, v = int(name_split[1]), int(name_split[2])
        wgt = G[u][v]["weight"]
        edges.append([u, v, wgt])
  #print(edges)
  file_name = folder_name + '/log_folder/' + file_name_without_ext + '_output'
  levels = 1
  n = len(list(G.nodes()))
  write_file(file_name, levels, [i for i in range(n)], edges, subset_arr)
  #return names, x
  return prob.solution.get_objective_value()
...
def test_single_flow_ilp(folder_name, file_name_without_ext, stretch_factor):
  ...
  solution_value = single_flow_ilp(G, subset_arr, multiplicative_check, stretch_factor, folder_name, file_name_without_ext)
