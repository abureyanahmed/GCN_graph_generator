...
def main(G, MAX_WEIGHT, MAX_COST, sourceNode, destNode):
	...
	#	weight = (float)(edgeData[2])
	#	#cost = (int)(edgeData[3])
	#	cost = (float)(edgeData[3])
	#	#print(edgeData)
	...
	while H:
		...
		if node == dest:
			...
			for item in adjlist:
				...
				newWeight = weight+(float)(G[node][v]['weight'])
				#newCost = cost+(int)(G[node][v]['cost'])
				newCost = cost+(float)(G[node][v]['cost'])

				#thisEdgeWeight = (int)(G[node][v]['weight'])
				thisEdgeWeight = (float)(G[node][v]['weight'])
				#thisEdgeCost = (int)(G[node][v]['cost'])
				thisEdgeCost = (float)(G[node][v]['cost'])
